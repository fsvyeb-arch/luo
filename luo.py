...
def fetch_data(etf_list, custom_divs):
    ...
    for item in etf_list:
        try:
            ...
            prev_close = float(hist['Close'].iloc[-2]) if len(hist) >= 2 else float(curr_p)
...
if st.session_state.show_pledge:
    if not df.empty:
        st.markdown("#### 🏦 股票質押專區 (維持率與利息監控)")
        st.info("💡 股票質押後會從一般券商庫存消失。最高可借出擔保品市值的 60%。股票質押通常為「按月繳息、到期還本」，一約通常為期 1.5 年 (18個月)。")
        
        pledge_data = st.session_state.my_data.get('pledge', {})
        def_borrowed = float(pledge_data.get('borrowed_amount', 200000))
        def_rate = float(pledge_data.get('interest_rate', 3.45))
        if def_borrowed == 0.0: def_borrowed = 200000.0
        if def_rate == 0.0: def_rate = 3.45
        
        col_p1, col_p2 = st.columns(2)
        with col_p1:
            borrowed = st.number_input("💸 輸入已向券商借入款項總額 (元)", min_value=0, value=int(def_borrowed), step=10000)
        with col_p2:
            interest_rate = st.number_input("📈 券商質押年利率 (%)", min_value=0.0, value=float(def_rate), step=0.01)
        
        if borrowed != def_borrowed or interest_rate != def_rate:
            st.session_state.my_data['pledge'] = {'borrowed_amount': borrowed, 'interest_rate': interest_rate}
            save_to_json(st.session_state.my_data)
            st.rerun()

        # 質押利息試算 (1.5年 = 18個月)
        monthly_interest = float(borrowed) * (float(interest_rate) / 100.0) / 12.0
        total_interest_18m = monthly_interest * 18.0
        
        st.markdown("##### 💵 質押利息試算 (一約為期 1.5 年 / 18個月)")
        c_i1, c_i2, c_i3 = st.columns(3)
        c_i1.metric("每月應繳利息 (純繳息)", f"${monthly_interest:,.0f}", "每月固定現金流支出", delta_color="inverse")
        c_i2.metric("一年半 (18期) 總利息", f"${total_interest_18m:,.0f}", "1.5年總利息成本", delta_color="inverse")
        c_i3.metric("到期應還本金", f"${borrowed:,.0f}", "期滿可向券商申請展延續約", delta_color="off")
        ...
        for item in st.session_state.my_data['etfs']:
            ...
            p_shares = float(item.get('pledged_shares', 0.0))
        ...
        margin_ratio = (total_pledge_mkt / float(borrowed) * 100.0) if borrowed > 0 else 0.0
        
        st.markdown("##### 📊 維持率與額度監控")
        col_m1, col_m2, col_m3, col_m4 = st.columns(4)
        col_m1.metric("擔保品總市值", f"${total_pledge_mkt:,.0f}")
        col_m2.metric("🎯 總可借款上限 (60%)", f"${total_borrowable:,.0f}")
        col_m3.metric("💸 預留額度", f"${total_borrowable - borrowed:,.0f}")
        ...
        st.markdown("👇 **直接勾選下方表格最前方的「☑️ 選取」方格，即可在下方設定該檔 ETF 的質押張數！**")
        ...
        edited_df = st.data_editor(
            ...
            hide_index=True,
            column_config={
                "☑️ 選取": st.column_config.CheckboxColumn("☑️ 選取", default=False)
            },
            disabled=["ETF 名稱", "總庫存 (張)", "質押張數", "現價", "質押市值 (元)", "可借上限 (60%)"]
        )
        
        selected_rows = edited_df[edited_df["☑️ 選取"] == True]
        
        if not selected_rows.empty:
            selected_pledge_etf = selected_rows.iloc[-1]['ETF 名稱']
            current_p_shares = selected_rows.iloc[-1]['質押張數']
            
            st.markdown(f"##### ✍️ 正在設定 【{selected_pledge_etf}】")
            
            col_s1, col_s2 = st.columns([1, 1])
            with col_s1:
                new_p_shares = st.number_input(f"請輸入質押張數", min_value=0.0, value=float(current_p_shares), step=1.0, key="input_pledge_shares")
            with col_s2:
                st.write("")
                st.write("")
                if st.button("💾 更新質押張數", use_container_width=True, type="primary"):
                    for item in st.session_state.my_data['etfs']:
                        if item['name'] == selected_pledge_etf:
                            item['pledged_shares'] = new_p_shares
                            save_to_json(st.session_state.my_data)
                            st.rerun()
        else:
            st.info("👆 請在上方表格最左側勾選您想要設定的標的。")
